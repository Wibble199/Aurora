            ...
            if (gamestate is GameState_ETS2 && (gamestate as GameState_ETS2).Truck.lightsBeaconOn) {
                switch (this.Properties.BeaconStyle) {
                    ...
                    case ETS2_BeaconStyle.Half_Alternating:
                        var half = frame < 5
                            ? Properties.Sequence.Keys
                            : Properties.Sequence.Keys.Skip((int)Math.Ceiling((double)Properties.Sequence.Keys.Count / 2));
                        half = half.Take((int)Math.Ceiling((double)Properties.Sequence.Keys.Count / 2));
